...
3. **Local env file** — `cp .env.example .env.local` and set at minimum:
   ...
   Login uses **usernames** **`admin`** or **`user`** (matched case-insensitively, e.g. `Admin` / `User`) with the **same** fixed password **`1234`** in [`api/login.ts`](api/login.ts) (POC only; not env-configurable). `admin` gets the full app shell; `user` sees only Report and Training (plus server blocks listing/exporting all incidents).
